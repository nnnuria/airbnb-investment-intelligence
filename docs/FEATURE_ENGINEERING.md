...
## 7. Price-model features — `notebooks/price_ml_model.ipynb`
...
> ℹ️ **Reproducibility note:** the **production** price model is the *by-city* LightGBM set (`price_city_*.pkl`), reproduced faithfully by `scripts/train_price_model.py` — a headless retrain that mirrors `notebooks/price_ml_model_comparison.ipynb` and matches the shipped combined Test R² 0.8137 / RMSE €68.5 exactly. The legacy *general* single model (`price_best_model.pkl`) is a fallback only; its older notebook `price_ml_model.ipynb` trains **HistGradientBoosting** (labelled "XGBoost-equivalent"), so re-running *that* notebook won't reproduce it — treat it as a narrated reference, not a retrain path.
...
| Price model | `has_reviews`, stay-length flags, target/integer encodings, ~29 selected | `price_ml_model.ipynb` → `models/price.py` |
...
**Two reproducibility/governance items to hand back to the team:** the multi-source config (§9) and the amenity-pipeline divergence (§2). *(The price-model retrain gap is now closed — see `scripts/train_price_model.py`, §7.)*
